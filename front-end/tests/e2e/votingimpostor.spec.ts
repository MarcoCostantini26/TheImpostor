...
test('players vote and eliminate the impostor (he submits wrong final guess)', async ({ browser }) => {
  ...
  await Promise.all(pages.map(async (p: import('@playwright/test').Page) => {
    ...
    if (me === detectedImpostor) {
      try {
        ...
        await guessInput.fill(WRONG_GUESS)
  ...
  const secretWord = found?.secret || (secrets[0]?.secret ?? null)
  console.log('SECRET WORD:', secretWord)
  console.log('IMPOSTOR WRONG GUESS:', WRONG_GUESS)
